const tableRowTemplate = (environment, id, ...args) =>
    `<tr data-row-id="${id}">
        <td ${environment === ENVIRONMENT_TYPE.COUNTRIES ? `onclick="showCountryCities(${id})"` : ''}>${args[0]}</td>
        <td>${args[1]}</td>
        <td>${args[2]}</td>
        <td>${args[3]}</td>
        <td class="actions">
            <div class="actions no-select">
                <svg width="20" height="20" viewBox="0 0 20 20" class="action" ${environment === ENVIRONMENT_TYPE.COUNTRIES ? `onclick="deleteCountry(${id})"` : environment === ENVIRONMENT_TYPE.CITIES ? `onclick="deleteCity(${id})"` : ''}
                    ...
                    <path d="M9.99993 6.63098C9.74001 6.63098 9.52927 6.84168 9.52927 7.10165V14.7827C9.52927 15.0426 9.73997 15.2533 9.99993 15.2533C10.2599 15.2533 10.4706 15.0426 10.4706 14.7827V7.10165C10.4706 6.84172 10.2599 6.63098 9.99993 6.63098Z"/>
                    <path d="M13.3455 6.6311C13.0866 6.62536 12.8709 6.83274 12.8659 7.09262L12.7182 14.7736C12.7132 15.0335 12.9198 15.2482 13.1797 15.2532C13.1828 15.2533 13.1859 15.2533 13.1889 15.2533C13.4446 15.2533 13.6544 15.0485 13.6593 14.7917L13.807 7.11071C13.812 6.85079 13.6054 6.63606 13.3455 6.6311Z"/>
                </svg>
    
                <svg width="1" height="26" viewBox="0 0 1 26" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
                    <line x1="0.5" y1="25.5" x2="0.500001" y2="0.5" stroke="#969696" stroke-linecap="round"/>
                </svg>
                <svg width="22" height="22" viewBox="0 0 22 22" class="action" onclick="edit(${id})"
                     xmlns="http://www.w3.org/2000/svg">
...
const orderArrow = (orderType) => {
    if (!orderType) return '';
    if (orderType === ORDER_TYPE.ASC) return downArrow();
    if (orderType === ORDER_TYPE.DESC) return upArrow();
}

const upArrow = () =>
    `<svg width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M7 4L4 1L1 4" stroke="#5C5C5C" stroke-width="1.2" stroke-linecap="round"/>
    </svg>`;

const downArrow = () =>
    `<svg width="8" height="5" viewBox="0 0 8 5" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M1 1L4 4L7 1" stroke="#5C5C5C" stroke-width="1.2" stroke-linecap="round"/>
    </svg>`;

const pageNumber = (pageNumber, isActive) =>
    `<a ${isActive ? 'class="badge"' : ''} onclick="changePage(${pageNumber})">${pageNumber}</a>`;